...
pub const Response = struct {
    ...
    pub fn useEventStream(self: *Self) void {
    ...
    pub fn write(self: *Self) !void {
        ...
        if (self.content_type == .EVENTS) {
            try stream.writeAll(": using event stream\n\n");
        }
    }
    ...
    fn writeHeaders(self: *Self, stream: Stream) !void {
        ...
        if (self.body) |body| {
            ...
            const fin = blk: {
                if (self.chunked) {
                    break :blk "Transfer-Encoding: chunked\r\n";
                }
                if (self.content_type == .EVENTS) {
                    break :blk "";
                }
